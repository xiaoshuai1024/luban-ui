...
const filteredPropEntries = computed(() => {
  ...
  return propEntries.value.filter(([key, schema]) => {
    ...
    if (schema.visibleWhen) {
      ...
      if (cond.notEquals !== undefined && actual === cond.notEquals)
        return false;
    }
    if (!q) return true;
    return (
      (schema.label ?? key).toLowerCase().includes(q) ||
...
const filteredStyleEntries = computed(() => {
  ...
  return styleEntries.value.filter(
    ([key, schema]) =>
      (schema.label ?? key).toLowerCase().includes(q) ||
      key.toLowerCase().includes(q),
  );
...
function optionsList(
  ...
  return Array.isArray(v)
    ? (v as { label: string; value: string | number }[])
    : [];
}
...
function updateOption(
  ...
  const list = optionsList(key, isStyle).map((o, i) =>
...
function removeOption(key: string, index: number, isStyle = false): void {
  patch(
    key,
    optionsList(key, isStyle).filter((_, i) => i !== index),
    isStyle,
  );
}
</script>
...
<template>
  <div class="lb-property-panel">
    <template v-if="nodeMeta && hasAnyField">
      <div class="lb-property-panel__header">
        {{ nodeMeta.label }} 属性配置
      </div>
      ...
      <!-- 基础属性（可折叠） -->
      <div
        v-if="filteredPropEntries.length"
        class="lb-property-panel__section"
      >
        <div
          class="lb-property-panel__section-title"
          @click="toggleGroup('基础')"
        >
          <span class="lb-property-panel__arrow">{{
            isCollapsed('基础') ? '▸' : '▾'
          }}</span>
          基础属性
        </div>
        <div v-show="!isCollapsed('基础')">
          <div
            v-for="[key, schema] in filteredPropEntries"
            :key="'p-' + key"
            class="lb-property-field"
          >
            <label
              class="lb-property-field__label"
              :class="{ 'lb-property-field__label--required': schema.required }"
            >
              {{ schema.label ?? key }}
            </label>
            <input
              v-if="setterFor(schema) === 'string'"
            ...
            >
            <input
              v-else-if="setterFor(schema) === 'number'"
              class="lb-property-input"
              type="number"
            ...
            >
            <label
              v-else-if="setterFor(schema) === 'boolean'"
              class="lb-property-switch"
            ><input
              type="checkbox"
              ...
              "
            ><span>{{
              Boolean(getValue(key, schema.default ?? false)) ? '是' : '否'
            }}</span></label>
            <select
              v-else-if="setterFor(schema) === 'select'"
              class="lb-property-input"
            ...
            <div
              v-else-if="setterFor(schema) === 'color'"
              class="lb-property-color"
            >
              <input
                type="color"
                class="lb-property-color__picker"
                :value="String(getValue(key, '#000000'))"
                @input="patch(key, ($event.target as HTMLInputElement).value)"
              ><input
                type="text"
                class="lb-property-input lb-property-color__text"
                :value="String(getValue(key, '#000000'))"
                @input="patch(key, ($event.target as HTMLInputElement).value)"
              >
            </div>
            <div
              v-else-if="setterFor(schema) === 'image'"
              class="lb-property-image"
            >
              <input
                class="lb-property-input"
                type="text"
              ...
              ><img
                v-if="getValue(key, '')"
                class="lb-property-image__preview"
                :src="String(getValue(key, ''))"
                alt="预览"
              >
            </div>
            <textarea
              v-else-if="setterFor(schema) === 'json'"
              class="lb-property-textarea"
              :value="jsonText(key)"
              rows="4"
              @blur="
                commitJson(key, ($event.target as HTMLTextAreaElement).value)
              "
            />
            <div
              v-else-if="setterFor(schema) === 'options'"
              class="lb-property-options"
            >
              <div
                v-for="(opt, i) in optionsList(key)"
                :key="i"
                class="lb-property-options__row"
              >
                <input
                ...
                ><input
                  ...
                  "
                ><button
                  class="lb-property-options__btn"
                  type="button"
              ...
              <button
                class="lb-property-options__add"
                type="button"
                @click="addOption(key)"
              >
                + 添加
      ...
      <!-- 样式属性（可折叠） -->
      <div
        v-if="filteredStyleEntries.length"
        class="lb-property-panel__section"
      >
        <div
          class="lb-property-panel__section-title"
          @click="toggleGroup('样式')"
        >
          <span class="lb-property-panel__arrow">{{
            isCollapsed('样式') ? '▸' : '▾'
          }}</span>
        ...
        <div v-show="!isCollapsed('样式')">
          <div
            v-for="[key, schema] in filteredStyleEntries"
            :key="'s-' + key"
            class="lb-property-field"
          >
            <label class="lb-property-field__label">{{
              schema.label ?? key
            }}</label>
            <input
              v-if="
                setterFor(schema) === 'string' ||
                  setterFor(schema) === 'spacing'
              "
              class="lb-property-input"
              type="text"
              :placeholder="schema.placeholder ?? '如 16px'"
              :value="String(getValue(key, schema.default ?? '', true))"
              @input="
                patch(key, ($event.target as HTMLInputElement).value, true)
              "
            >
            <div
              v-else-if="setterFor(schema) === 'color'"
            ...
            >
              <input
                type="color"
                class="lb-property-color__picker"
                :value="String(getValue(key, '#000000', true))"
                @input="
                  patch(key, ($event.target as HTMLInputElement).value, true)
                "
              ><input
            ...
            <select
              ...
              @change="
                patch(key, ($event.target as HTMLSelectElement).value, true)
              "
            >
              <option
                v-for="opt in schema.options"
            ...
            <input
              ...
              class="lb-property-input"
              type="text"
              :value="String(getValue(key, '', true))"
              @input="
                patch(key, ($event.target as HTMLInputElement).value, true)
              "
            >
    ...
    <div
      v-else
      class="lb-property-panel__empty"
    >
      <div class="lb-property-panel__empty-icon">
        ⚙️
      </div>
...
.lb-property-panel__section {
  padding: 8px 16px;
  border-bottom: 1px solid #f5f5f5;
}

.lb-property-panel__section-title {
  font-size: 11px;
  color: #909399;
...
.lb-property-panel__arrow {
  font-size: 10px;
  color: #c0c4cc;
}

.lb-property-panel__search {
  padding: 8px 12px;
}

.lb-property-panel__search-input {
  width: 100%;
  padding: 6px 10px;
...
.lb-property-panel__search-input:focus {
  border-color: #409eff;
}
...
.lb-property-color__text {
  flex: 1;
}

.lb-property-image__preview {
  margin-top: 4px;
  max-width: 100%;
  max-height: 80px;
  border-radius: 4px;
