...
const safeIndex = computed(() =>
  slideCount.value > 0 ? current.value % slideCount.value : 0,
);
...
<template>
  ...
  >
    ...
    <button
      v-if="slideCount > 1"
      class="lb-carousel__nav lb-carousel__nav--next"
      type="button"
      @click="next"
    >
      ›
    ...
    <div v-if="slideCount > 1" class="lb-carousel__dots">
      <button
        ...
        :class="[
          'lb-carousel__dot',
          { 'lb-carousel__dot--active': i === safeIndex },
        ]"
        @click="current = i"
      />
    </div>
...
.lb-carousel__nav--prev {
  left: 8px;
}
.lb-carousel__nav--next {
  right: 8px;
}
.lb-carousel__dots {
  position: absolute;
